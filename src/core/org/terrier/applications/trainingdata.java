...
static double lamda=0.3;
	...
	BufferedWriter bw1=new BufferedWriter(fw1);
		...
		int topic=0;
		try (DirectoryStream<Path> stream = Files.newDirectoryStream(dirtopic)) {System.out.println("top");
		    for (Path folder: stream){System.out.println("middle");
		    ArrayList<sentencescore> s=new ArrayList<sentencescore>();
		    	 DirectoryStream<Path> stream1 = Files.newDirectoryStream(folder);
				    for (Path file: stream1){System.out.println("inner");
